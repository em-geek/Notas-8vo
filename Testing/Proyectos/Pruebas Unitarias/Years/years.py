...
class Years:
    ...
    def _validar_fecha(self, fecha):
        ...
        """
        if not fecha:
            return None, 'La entrada no debe de estar vacia'
        partes = fecha.split('/')
        if len(partes) != 3:
            return None, 'El formato proporcionado no se detecta como una fecha DD/MM/AAAA'
        try:
            dia, mes, anio = map(int, partes)
        except ValueError:
            return None, 'El dia, mes y año debe estar escrito en numero'
        if anio < 1800:
            return None, 'Fecha invalida, el año no debe ser menor a 1800.'
        hoy = datetime.today()
        if anio > hoy.year or (anio == hoy.year and mes > hoy.month) or (anio == hoy.year and mes == hoy.month and dia > hoy.day):
            return None, 'La fecha debe de estar dentro del rango desde 1800 hasta la fecha actual'
        return (dia, mes, anio), None

    def _dias_en_mes(self, mes, anio):
        """Retorna la cantidad de días del mes para el año dado."""
        dias_mes = {
            1: 31, 2: 29 if self.es_bisiesto(anio) else 28, 3: 31, 4: 30, 5: 31, 6: 30,
            7: 31, 8: 31, 9: 30, 10: 31, 11: 30, 12: 31
        }
        return dias_mes.get(mes)
    ...
    def convertir_fecha(self, fecha):
        ...
        """
        datos, error = self._validar_fecha(fecha)
        if error:
            return error
        dia, mes, anio = datos
        if mes not in self.MESES:
            return 'El mes proporcionado no existe'
        dias_mes = self._dias_en_mes(mes, anio)
        if dia < 1 or dia > dias_mes:
            return 'El dia proporcionado no existe'
        texto = f"{self.NUMEROS[dia]} de {self.MESES[mes]} de {self.convertir_anio(anio)}"
        return texto

    def convertir_anio(self, anio):
        """
        ...
        if anio == 2000:
            return "dos mil"
        if anio < 2000:
            mil, resto = "mil", anio - 1000
        else:
            mil, resto = "dos mil", anio - 2000
        resto_texto = self._convertir_resto_anio(resto)
        return f"{mil} {resto_texto}".strip()

    def _convertir_resto_anio(self, anio):
        """
